...
async function applyRankByRelevance(
  ...
  ctx.llmCalls += 1;
  if (metricsEnabled()) queryLlmCallsTotal.inc({ aggregator: "rank_by_relevance" });
...
async function executeStep(
  ...
  switch (step.op) {
    ...
    case "synthesize": {
      ...
      ctx.llmCalls += 1;
      if (metricsEnabled()) queryLlmCallsTotal.inc({ aggregator: "synthesize" });
  ...
  for (const step of pipeline) {
    ...
    try {
      result = await withSpan(
        `query.step.${step.op}`,
        {
          "agentmemory.step_op": step.op,
          "agentmemory.step_id": step.id,
          "agentmemory.in_count": inCount,
          "agentmemory.cost_class": cost,
        },
        async (span) => {
          const r = await executeStep(step, streams, ctx, depth);
          if (r.kind === "terminal") {
            span.setAttribute("agentmemory.out_count", 0);
            span.setAttribute("agentmemory.terminal", true);
          } else if (r.kind === "error") {
            span.setAttribute("agentmemory.outcome", "error");
          } else {
            span.setAttribute("agentmemory.out_count", r.output.length);
          }
          return r;
        },
      );
    ...
    ctx.budget.spent += cost;
    if (metricsEnabled()) queryStepsTotal.inc({ op: step.op, status: "success" });
    trace.push({
      op: step.op,
      stepId: step.id,
      inCount,
...
async function executePipeline(
  ...
  const inner = await withSpan(
    "mem::query.pipeline",
    {
      "agentmemory.pipeline_length": pipeline.length,
      "agentmemory.budget_cap": ctx.budget.cap,
    },
    async (span) => {
      const r = await executePipelineInternal(pipeline, ctx, 0);
      span.setAttribute("agentmemory.result_kind", r.kind);
      span.setAttribute("agentmemory.budget_spent", ctx.budget.spent);
      span.setAttribute("agentmemory.llm_calls", ctx.llmCalls);
      return r;
    },
  );
  if (metricsEnabled()) {
    queryBudgetUsed.observe(ctx.budget.spent);
    queryPipelinesTotal.inc({ result_kind: inner.kind });
    if (inner.kind === "error" && /deadline_exceeded/.test(inner.error)) {
      queryDeadlineExceededTotal.inc();
    }
  }
  if (inner.kind === "error") {
    return {
  ...
  sdk.registerFunction(
    ...
    async (data: QueryRequest): Promise<QueryResult> => {
      ...
      if (opts.dry_run === true) {
        if (metricsEnabled()) {
          queryPipelinesTotal.inc({ result_kind: "dry_run" });
        }
        return {
          kind: "dry_run",
          plan: pipeline,
