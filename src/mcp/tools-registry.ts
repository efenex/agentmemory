...
export const V050_TOOLS: McpToolDef[] = [
  ...
  {
    name: "memory_action_get",
    description:
      "Read a single action's full record by ID: title, description, status, result, priority, tags, plus its dependency edges and child actions. Use this to retrieve the complete body of an act_… item that frontier/next/query only surface as a truncated title. Accepts a full id or a truncated act_<ts> prefix (as handoffs sometimes record) and resolves it to the unique match.",
    inputSchema: {
      type: "object",
      properties: {
        actionId: {
          type: "string",
          description:
            "Action ID to fetch — full (act_<ts>_<hex>) or truncated act_<ts> prefix",
        },
      },
      required: ["actionId"],
    },
